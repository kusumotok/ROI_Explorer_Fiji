...
public class RoiExplorerWindow extends JFrame implements RoiEditController.EditHost {
    ...
    public RoiExplorerWindow() {
        ...
        addWindowListener(new WindowAdapter() {
            ...
            public void windowClosing(WindowEvent e) {
                ...
                cancelSplitMode();
                OpenViewRegistry.getInstance().unregister(RoiExplorerWindow.this);
                dispose();
            }
        });
    ...
    public void openFolder(Path path) {
        ...
        setTitle("ROI Explorer – " + path.getFileName());
        updateStatus();
        refreshOverlay();
    }
    ...
    private void cmdMoveToFolder() {
        ...
        if (vr == null) return;
        MoveToFolderDialog dlg = new MoveToFolderDialog(this, vr, sel);
        dlg.setVisible(true);
    ...
    private boolean cmdSetGroupMeasureOptions() {
        GroupMeasurementDialog dlg = new GroupMeasurementDialog(this, boundImage, groupMeasureOptions);
        dlg.setVisible(true);
    ...
    private void cmdProperties() {
        ...
        RoiPropertiesDialog dlg = new RoiPropertiesDialog(this, rois);
        dlg.setVisible(true);
    ...
    private void positionWindow() {
        java.util.List<RoiExplorerWindow> windows = OpenViewRegistry.getInstance().getWindows();
        if (windows.isEmpty()) {
            setLocationRelativeTo(null);
            return;
        }
        RoiExplorerWindow anchor = windows.get(windows.size() - 1);
        if (anchor == this) {
            setLocationRelativeTo(null);
            return;
        }
        Rectangle bounds = GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds();
        int x = anchor.getX() + WINDOW_STAGGER;
        int y = anchor.getY() + WINDOW_STAGGER;
